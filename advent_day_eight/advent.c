...
void get_curr_antinodes(int antennas_pos[ANTENNAS][2], int antennas_count, int antinodes_pos[LOCATIONS][2], int* antinode_count) {
    for (int i = 0; i < antennas_count; i++) {
        for (int j = 0; j < antennas_count; j++) {
            ...
            int rel_dist[2] = {dist[0] * 2, dist[1] * 2};
            int antinode_pos[2] = {antennas_pos[i][0] + rel_dist[0], antennas_pos[i][1] + rel_dist[1]};
            if (is_in_bounds(antinode_pos[0], antinode_pos[1])) {
                if (!has_antinode(antinode_pos, antinodes_pos, *antinode_count)) {
                    antinodes_pos[*antinode_count][0] = antinode_pos[0];
                    antinodes_pos[*antinode_count][1] = antinode_pos[1];
                    (*antinode_count)++;
                }
            }
        }
...
int main() {
    ...
    printf("Advent day eight part one result is %d", antinode_count);
}